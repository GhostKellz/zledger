...
pub const WalletKeypair = struct {
    public_key: [32]u8,
    private_key: [64]u8,  // Use 64 bytes to accommodate ed25519
    ...
    pub fn generate(algorithm: SignatureAlgorithm) WalletKeypair {
        switch (algorithm) {
            ...
            .secp256k1 => {
                ...
                var padded_private_key: [64]u8 = std.mem.zeroes([64]u8);
                @memcpy(padded_private_key[0..32], &keypair.private_key);
                return WalletKeypair{
                    .public_key = keypair.public_key_x,
                    .private_key = padded_private_key,
                    .algorithm = algorithm,
                };
            },
        }
    ...
    pub fn signTransaction(self: *TransactionSigner, allocator: std.mem.Allocator, transaction: *tx.Transaction) !void {
        ...
        switch (self.keypair.algorithm) {
            .ed25519 => {
                ...
                transaction.signature = try ed_keypair.sign(tx_data);
            },
            .secp256k1 => {
                const secp_keypair = zcrypto.asym.Secp256k1KeyPair{
                    .public_key_x = self.keypair.public_key,
                    .public_key_compressed = undefined, // Not needed for signing
                    .private_key = self.keypair.private_key[0..32].*,
                };
                const tx_hash = zcrypto.hash.sha256(tx_data);
                transaction.signature = try secp_keypair.sign(tx_hash);
            },
        }
    ...
    pub fn verifyTransaction(self: *TransactionSigner, allocator: std.mem.Allocator, transaction: tx.Transaction) !bool {
        ...
        switch (self.keypair.algorithm) {
            ...
            .secp256k1 => {
                const secp_keypair = zcrypto.asym.Secp256k1KeyPair{
                    .public_key_x = self.keypair.public_key,
                    .public_key_compressed = undefined, // Not needed for verification
                    .private_key = undefined, // Not needed for verification
                };
                const tx_hash = zcrypto.hash.sha256(tx_data);
                return secp_keypair.verify(tx_hash, transaction.signature.?);
            },
        }
    }
